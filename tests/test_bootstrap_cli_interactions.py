...
"""Interactive branches in the ``prodockit bootstrap --apply`` command loop."""
...
def test_bootstrap_profile_is_passed_while_the_command_context_is_built(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """The Windows SSH choice lives in the runner built with the context.

    Marking a completed context as guided afterwards changes its display
    ...
    def build(config, *, guided=False):  # type: ignore[no-untyped-def]
        received.append(guided)
        return build_context(
            config,
            runner=CliFakeRunner(),
            platform=MACOS,
    ...
    result = CliRunner().invoke(
        main,
        ["bootstrap", "--check", "--config", str(tmp_path / "pdk.toml")],
    )
...
def test_every_real_stage_has_one_forward_moving_phase() -> None:
    phases = [_bootstrap_phase(stage.id) for stage in STAGES]
...
def test_bootstrap_creates_the_host_project_before_choosing_and_cloning() -> None:
    """A newly created empty repository becomes usable in the same run."""
    bootstrap_ids = [stage.id for stage in STAGES]

    assert bootstrap_ids.index("ssh-upload") < bootstrap_ids.index("own-project")
    assert bootstrap_ids.index("own-project") < bootstrap_ids.index("clone-source")
    assert bootstrap_ids.index("clone-source") < bootstrap_ids.index("clone")
    assert bootstrap_ids.index("clone") < bootstrap_ids.index("fresh-history")
...
def test_bootstrap_prints_captured_output_when_a_command_fails(tmp_path: Path) -> None:
    runner = CliFakeRunner(
        {"installer": CommandResult(1, stdout="download context", stderr="package failed")}
    )
    context = build_context(
