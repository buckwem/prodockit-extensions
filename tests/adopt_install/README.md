...
The separate **Release real upgrades** workflow runs
`tools/adopt_native_upgrade.py` on all five runner and architecture
combinations. That slower gate installs published
...
It runs for a release pull request, when its own gate changes, or when requested
manually. It is not repeated for ordinary Adopt changes or after the release
merge; the normal installed-wheel matrix provides the quicker change-level coverage.
