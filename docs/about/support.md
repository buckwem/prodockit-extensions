...
Windows x64, and macOS ARM64. Windows ARM64 remains supported for ordinary
Prodockit use, but not for `--swap`, because the audited `quickjs-ng` version
does not publish a Windows ARM64 wheel and normal installation must not compile
or download another runtime.
