...
- Made Template Sync replace complete multiline TOML arrays without leaving stale lines behind.
...
- Kept PDF table rowspan groups intact across page boundaries so shaded spanning cells retain their full background.
...
- Made `prodockit.tables` reject missing separators, stray tokens and unbalanced quotes in supported cell attribute lists with source-line diagnostics.
...
- Made Bootstrap defer optional PDF prerequisite checks to the first `pdk pdf` instead of requiring macOS Pango during setup.
