...
| `renderer.mermaid` | Authored Markdown uses a Mermaid fence but neither the project-local `mmdc` nor a usable command on `PATH` exists. Preserving Zensical's unused Mermaid default does not make the renderer required. | Run `pdk init-tools` from the project root. If `pdf_mmdc_bin` is explicit, correct it to the generated script or executable. Confirm the reported path exists. |
