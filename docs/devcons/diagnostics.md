...
| `maintenance.adopt-readiness` | Diagnostics found the same local supported-toolchain or standard-component work that `pdk adopt` and the prerequisite phase of `template-sync` would report. A warning means bounded integration remains; a failure means Adopt found a blocker. This check is local and independent of `prodockit-template`. | Run `pdk adopt --dry-run` to review the matching stages, then `pdk adopt --apply`. Rerun `pdk diag`; it should pass this check before `template-sync` introduces unrelated template changes. An offline diagnostic cannot report a remote template update; use `pdk diag --online` or `pdk template-sync` for that separate comparison. |
