...
| `renderer.mermaid` | Mermaid is configured but neither the project-local `mmdc` nor a usable command on `PATH` exists. | Run `pdk init-tools` from the project root. If `pdf_mmdc_bin` is explicit, correct it to the generated script or executable. Confirm the reported path exists. |
...
| `renderer.mathjax` | `tools/mathjax/tex2svg.js` or the installed `mathjax-full` inputs are missing while maths rendering is configured. | Run `pdk init-tools` to install the pinned Node inputs. If only website JavaScript is absent, run `pdk init-mathjax` after the install. Do not hand-edit generated vendor files. |
