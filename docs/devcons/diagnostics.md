...
| `maintenance.adopt-readiness` | Diagnostics found the same local supported-toolchain, standard-component, or saved component-choice work that `pdk adopt` and the prerequisite phase of `template-sync` would report. Standard-component details name only the missing extensions or website inputs. An existing bibliography-backed or inline citation implementation satisfies the citation capability without enabling the alternative. A warning means bounded integration remains; a failure means Adopt found a blocker. This check is local and independent of `prodockit-template`. | Run `pdk adopt --dry-run` to review the matching stages, then `pdk adopt --apply`. A missing `.prodockit-components.toml` appears as its own manifest-only stage so that saving inferred Mermaid and maths choices is not confused with changing extensions or styles. Rerun `pdk diag`; it should pass this check before `template-sync` introduces unrelated template changes. An offline diagnostic cannot report a remote template update; use `pdk diag --online` or `pdk template-sync` for that separate comparison. |
