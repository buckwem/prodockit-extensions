...
| An established back-of-book index preparation can no longer import PyMuPDF | Rerun `pdk pdf` in the active project environment. A clean project defers PyMuPDF until its first indexed PDF build. |
...
| `maintenance.adopt-readiness` | Diagnostics found the same local supported-toolchain, standard-component, or saved component-choice work that `pdk adopt` and the prerequisite phase of `template-sync` would report. Standard-component details name only the missing extensions or website inputs. An existing bibliography-backed or inline citation implementation satisfies the citation capability without enabling the alternative. A warning means bounded integration remains, or Python 3.10–3.13 is in use: these versions have compatibility tests, but full Adopt toolchain integration is qualified on Python 3.14. Other Adopt blockers remain failures. This check is local and independent of `prodockit-template`. | For bounded integration, run `pdk adopt --dry-run` to review the matching activities, then `pdk adopt --apply`. Under Python 3.10–3.13, activate a Python 3.14 project environment before using Adopt to change packages or files; ordinary Prodockit use can continue. A missing `.prodockit-components.toml` appears as its own manifest-only activity and defaults both optional renderers off; use `pdk adopt --configure` first when the project needs Mermaid or maths. Rerun `pdk diag`; it should pass this check before `template-sync` introduces unrelated template changes. An offline diagnostic cannot report a remote template update; use `pdk diag --online` or `pdk template-sync` for that separate comparison. |
