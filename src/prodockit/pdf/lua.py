...
def build_lua_filter(
    ...
    return (
        ...
        "  if not ok then error('MathJax ' .. mode .. ' rendering failed: ' .. tostring(svg)) end\n"
        "  if not svg or svg == '' then error('MathJax ' .. mode .. ' rendering produced no SVG') end\n"
        "  return svg\n"
        "end\n\n"
        "local function write_math_svg(mode, source)\n"
        "  math_counter = math_counter + 1\n"
        "  local svg = render_math(mode, source)\n"
        "  local svg_path = math_dir .. '/formula_' .. math_counter .. '.svg'\n"
        "  local out, open_error = io.open(svg_path, 'w')\n"
        "  if not out then error('MathJax cannot write ' .. svg_path .. ': ' .. tostring(open_error)) end\n"
        "  local written, write_error = out:write(svg)\n"
        "  if not written then out:close(); error('MathJax cannot write ' .. svg_path .. ': ' .. tostring(write_error)) end\n"
        "  local closed, close_error = out:close()\n"
        "  if not closed then error('MathJax cannot close ' .. svg_path .. ': ' .. tostring(close_error)) end\n"
        "  return svg_path\n"
        "end\n\n"
        ...
        "-- the reader misclassify a fenced block as reflowing inline code.\n"
        ...
        "  local highlighted = el.attributes['prodockit-highlight']\n"
        "  if not highlighted or highlighted == '' then return nil end\n"
        "  local markup = decode_hex(highlighted)\n"
        "  return pandoc.RawBlock('html', "
        "'<div class=\"highlight prodockit-highlight\"><pre><code>' .. "
        "markup .. '</code></pre></div>')\n"
        "end\n\n"
        "function Div(el)\n"
        ...
        "    local svg_path = write_math_svg('display', text)\n"
        ...
        "    if not mathjax_available then return nil end\n"
        "    local text = pandoc.utils.stringify(el.content):gsub('^%s*\\\\%(%s*', ''):gsub('%s*\\\\%)%s*$', '')\n"
        "    local svg_path = write_math_svg('inline', text)\n"
        ...
        "  local svg_path = write_math_svg(is_display and 'display' or 'inline', el.text)\n"
