...
def build_lua_filter(
    ...
    return (
        ...
        "        -- prodockit.pdf.build computes every appendix page's letter\n"
        "        -- up front and stamps it here (see prodockit.pdf.html), so a\n"
        "        -- page contributing no numbered h1 still consumes its letter\n"
        "        -- and this stays in step with the website's own lettering\n"
        "        -- (prodockit._zensical.prescan_headings). Counting here\n"
        "        -- instead drifted whenever that happened - see issue #104.\n"
        "        -- Pandoc strips the 'data-' prefix off the attribute name.\n"
        "        local supplied = block.attributes['appendix-letter']\n"
        "        if supplied and supplied ~= '' then\n"
        "          appendix_index = string.byte(supplied) - 64\n"
        "        else\n"
