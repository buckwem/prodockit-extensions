...
def pdf(
    ...
    if prepare_components:
        try:
            ...
            if (sys.platform != "win32" and requested.intersection({"weasyprint", "all"})) or (
                "all" in requested and include_index
            ):
                python_prepared = (
                    prepare_pdf_python_requirements(config_file, include_index=include_index)
                    if "all" in requested
                    else prepare_pdf_python_requirements(config_file)
                )
